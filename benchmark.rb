...
module Benchmark
  ...
  class Gem
    ...
    def benchmark_operations(report_data)
      ...
      puts "\t\t#{gem_name} client init avg: #{format('%.2f', (values.sum(0.0) / values.size))} ms"
      ...
      operation_benchmarks.each do |test_name, test_def|
        ...
        puts "\t\t#{test_name} avg: #{format('%.2f',
                                             (values.sum(0.0) / values.size))} ms\tmem_allocated: #{'%.2f' % mem_allocated} kb"
      end
    end
